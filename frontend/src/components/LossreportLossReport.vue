<template>
    <v-card outlined>
        ...
        <v-card-title v-if="value._links">
            분실신고관리 # {{decode(value._links.self.href.split("/")[value._links.self.href.split("/").length - 1])}}
        </v-card-title >
        <v-card-title v-else>
            분실신고관리
        </v-card-title >        
	...
	async created() {
        ...
        methods: {
            ...
            async reportLoss() {
                try {
                    if(!this.offline){
                        var temp = await axios.post(axios.fixUrl(this.value._links['/reportloss'].href))
                        for(var k in temp.data) this.value[k]=temp.data[k];
                    }
